...
module.exports = class NeeoDevice extends EventEmitter {
        ...
        this._neeoDevice = 
            NeeoAPI.buildDevice(name, uniqueId)
                ...
                .registerDeviceSubscriptionHandler({
                    deviceAdded: (deviceId) => this.onDeviceAdded(deviceId),
                    deviceRemoved: (deviceId) => this.onDeviceRemoved(deviceId),
                    initializeDeviceList: (deviceIds) => this.onInitializeDeviceList(deviceIds),
                });
    }
    ...
    onButtonPressed(name, deviceId) {
        ...
        switch (name) {
            ...
            case 'FORWARD':
                this.onForward(deviceId);
                break;

            case 'REVERSE':
                this.onReverse(deviceId);
                break;
    ...
    addTextLabel(name, label, initialValue = '') {
        ...
        this._neeoDevice.addTextLabel({ name: name, label: label, isLabelVisible: false }, (deviceId) => { return this.getValue(deviceId, name) });
